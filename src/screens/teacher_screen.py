...
import pandas as pd
from src.screens.components.dialog_attendance_to_result import attendance_result_dialog
from src.screens.components.dialog_vioce_attendance import voice_attendance_dialog
...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning("Your havent created any subjects yet! Please create one to begin")    
        return 
    
    subject_options = {f"{s['name']}-{s['subject_code']}": s['subject_id'] for s in subjects}

    col1 , col2 = st.columns((3,1),vertical_alignment='bottom')  # means col1 is 3 times bigger than the col2

    with col1:
       selcted_subject_label =  st.selectbox("Select Subject",options=list(subject_options.keys()))

    with col2:
        if st.button("Add Photos",type='primary',icon=":material/photo_prints:",width='stretch'):
            add_photos_dialog()

    selected_subject_id = subject_options[selcted_subject_label]    
            ...
            with st.spinner("Deep scanning classroom photos....."):
                ...
                if not enroll_students:
                    ...
                    for node in enroll_students:
                        ...
                        source = all_detected_ids.get(int(student['student_id']),[])   # student aya toh hai but konsi so photo se aya hai if not in any photo then []
...
def teacher_tab_attendance_records():
    ...
    display_df = (
        ...
        [['time', 'subjects', 'subject_code', 'Attendance Stats']]
    )
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username= st.text_input("Enter username",placeholder='Anmol Rai')
    ...
    with btnc2:
       if st.button("Register Instead",icon=':material/passkey:',width='stretch',type='primary'):
              st.session_state.teacher_login_type="register"  # or we can also write it as  st.session_state['teacher_login_type']="register"
...
def register_teacher(teacher_username,teacher_name,teacher_pass,teacher_confirm_pass):
    ...
    try:
        ...
        return True,"successfully creatyed! login now"
...
def teacher_screen_register():
    ...
    teacher_username= st.text_input("Enter username",placeholder='AnmolRai')
    teacher_name= st.text_input("Enter name",placeholder='Anmol Rai')
    teacher_pass= st.text_input("Enter password",type='password',placeholder='Enter password')
